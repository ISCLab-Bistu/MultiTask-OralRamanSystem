...
class ClsHead(BaseHead):
    ...
    def loss(self, cls_score, labels, loss, **kwargs):
        num_samples = len(cls_score)
        losses = dict()
        # compute loss
        self.compute_loss = build_loss(loss)
        loss = self.compute_loss(
            cls_score, labels, avg_factor=num_samples, **kwargs)
        if self.cal_acc:
            # compute accuracy
            acc = self.compute_accuracy(cls_score, labels)
    ...
    def forward_train(self, cls_score, labels, **kwargs):
        if isinstance(cls_score, tuple):
            cls_score = cls_score[-1]

        losses = self.loss(cls_score, labels, **kwargs)
    ...
    def simple_test(self, cls_score, softmax=True, post_process=True):
        ...
        if len(cls_score.shape) == 1:
            cls_score = torch.unsqueeze(cls_score, dim=0)
        if softmax:
            pred = (
