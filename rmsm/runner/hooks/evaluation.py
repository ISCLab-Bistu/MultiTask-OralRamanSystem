...
from rmsm.utils.fileio import FileClient
from rmsm.utils import is_seq_of
...
class EvalHook(Hook):
    ...
    Args:
        ...
        save_best (str, optional): If a metric is specified, it would measure
            ...
            on the test dataset. e.g., ``AR@100`` for proposal
        ...
        test_fn (callable, optional): test a model with samples from a
            ...
            test function ``rmsm.utils.single_gpu_test`` will be used.
            (default: ``None``)
        ...
        file_client_args (dict): Arguments to instantiate a FileClient.
            See :class:`rmsm.utils.fileio.FileClient` for details. Default: None.
            `New in version 1.3.16.`
        **eval_kwargs: Evaluation arguments fed into the evaluate function of
            the dataset.

    Note:
        If new arguments are added for EvalHook, tools/txt_tocsv.py,
    ...
    def _do_evaluate(self, runner):
        ...
        results = self.test_fn(runner.model, self.dataloader)  # 当test_fn为None时候，rmsm.utils.single_gpu_test会被使用
        runner.log_buffer.output['eval_iter_num'] = len(self.dataloader)
        key_score = self.evaluate(runner, results)  # 输出测试集相应的分数
    ...
    def evaluate(self, runner, results):
        ...
        Args:
            runner (:obj:`rmsm.Runner`): The underlined training runner.
        ...
        if self.save_best is not None:
            ...
            # use f1_score to evaluate
            eval_results = (eval_res['f1_score'] + eval_res['f1_score2'] + eval_res['f1_score3']) / 3
...
class DistEvalHook(EvalHook):
    ...
    Args:
        ...
        test_fn (callable, optional): test a model with samples from a
            ...
            ``None``, the default test function ``rmsm.utils.multi_gpu_test``
            will be used. (default: ``None``)
    ...
    def __init__(self,
        ...
        if test_fn is None:
            from ..engine import multi_gpu_test
