...
from rmsm.utils import (IS_NPU_AVAILABLE, TORCH_VERSION, _BatchNorm,
                        digit_version)
...
try:
    ...
    # and used; otherwise, auto fp16 will adopt rmsm's implementation.
    if IS_NPU_AVAILABLE:
...
class OptimizerHook(Hook):
    ...
    def after_train_iter(self, runner):
        ...
        # 动态调整权重损失
        # loss1 = runner.outputs[0]['loss']
        # loss2 = runner.outputs[1]['loss']
        # loss3 = runner.outputs[2]['loss']
        # loss_weight1 = 3 * loss1 / (loss1 + loss2 + loss3)
        # loss_weight2 = 3 * loss2 / (loss1 + loss2 + loss3)
        # loss_weight3 = 3 * loss3 / (loss1 + loss2 + loss3)
        #
        # loss = loss_weight1 * runner.outputs[0]['loss'] + loss_weight2 * runner.outputs[1]['loss'] + loss_weight3 * \
        #        runner.outputs[2]['loss']

        loss = runner.outputs[0]['loss'] + runner.outputs[1]['loss'] + runner.outputs[2]['loss']
...
if (TORCH_VERSION != 'parrots'
    ...
    class Fp16OptimizerHook(OptimizerHook):
        ...
        Args:
            loss_scale (float | str | dict): Scale factor configuration.
                ...
                Defaults to 512. For Pytorch >= 1.6, rmsm uses official
    ...
    class Fp16OptimizerHook(OptimizerHook):  # type: ignore
        """FP16 optimizer hook (rmsm's implementation).
    ...
    class GradientCumulativeFp16OptimizerHook(  # type: ignore
        ...
        """Fp16 optimizer Hook (using rmsm implementation) implements multi-
